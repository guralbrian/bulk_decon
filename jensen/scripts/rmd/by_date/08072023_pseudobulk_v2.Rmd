...
gene.probs <- sweep(gene.probs,2,colSums(gene.probs),`/`)
...
pb.comps <- sapply(1:nrow(cell.ratios), function(x){MakePseudoBulk(cell.ratios[x,], gene.probs, 30000000, 0.7)})
...
pb.refs <- sapply(1:nrow(cell.ratios.ref), function(x){MakePseudoBulk(cell.ratios.ref[x,], gene.probs, 30000000, 0.7)})
...
cur.ratios <- cell.ratios |>
              subset(pct.diff == 30 & major.type == "fibroblasts")
cur.pb <- pb.comps |> 
          as.data.frame() |> 
...
false_pos <- matrix(results, ncol = 5) |>
              as.data.frame(
              )

rownames(false_pos) <- unique(all_combinations$pct.diff)
colnames(false_pos) <- unique(all_combinations$major.type)
false_pos$diff <- unique(all_combinations$pct.diff)

de_melt <- melt(false_pos, id.vars = "diff")

```

```{r plot false positives}

ggplot(data = de_melt, aes(x = diff, y = value, color = variable)) +
  geom_point() + 
