...
pb.comps <- sapply(1:nrow(cell.ratios), function(x){MakePseudoBulk(cell.ratios[x,], gene.probs, umi.target, 0.7)})
...
ref.pt <- 1/nrow(cell.ratios.ref)
dif.pt <- max.pt - ref.pt
levels.pt <- as.numeric(samp.reps*dif.pt/step.range)

cell.ratios <- as.data.frame(cell.ratios)
cell.ratios$pct.diff <- as.numeric(rep(seq(0, dif.pt*100, step.range*100), each = 25))
cell.ratios$major.type <- c(rep(NA, 25), rep(as.character(cell.types), times = as.integer(levels.pt+1))) # this is broken somehow. Using levels.pt gives n-1, but hardcoding the value of levels.pt works
