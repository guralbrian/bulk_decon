...
```{r plot raw, fig.width = 14, fig.height= 12}

plot.raw <- ggplot(comparison.clr, aes(x = log2FoldChange.raw, y = -log10(padj.raw), color = p_diff)) +
  geom_point(alpha = 0.8, size = 4) +
  scale_color_distiller(type = "div", limit = limit)+
  geom_text_repel(data = gained.clr, aes(label = Row.names), size = 8, box.padding = unit(0.35, "lines"), 
                  force = 40, segment.linetype = 2, segment.size = 0.6, color = "orange", force_pull = 0.01, min.segment.length = 0) + # label most increase sig genes 
...
ui <- fluidPage(
  ...
  sidebarLayout(
    sidebarPanel(
      selectInput("gene", "Select a Gene:", choices = comparison.clr$Row.names)
    ),
    mainPanel(
      plotOutput("genePlot")
    )
...
server <- function(input, output) {
  output$genePlot <- renderPlot({
    ...
    gene_data <- filter(comparison.clr, Row.names == selected_gene)
    
    # Create a plot (modify this as needed for your data)
    ggplot(comparison.clr, aes(x = log2FoldChange.clr, y = -log10(padj.clr), color = gene_data$Row.names)) +
        geom_point(alpha = 0.8, size = 4) +
        #scale_color_distiller(type = "div", limit = limit)+
        geom_text_repel(data = gained.clr, aes(label = Row.names), size = 8, box.padding = unit(0.35, "lines"), 
                  force = 40, segment.linetype = 2, segment.size = 0.6, color = "orange", force_pull = 0.01, min.segment.length = 0) + # label most increase sig genes 
