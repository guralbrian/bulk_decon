...
simulate_ratios <- function(major_cell, changing_cell, major_prop, changing_prop, 
  ...
  props[,3] <- props[,3] * sample(rnorm(1000, 1, 0.005), length(changing_props))
...
counts <- pb.add
counts.small <- counts[sample(rownames(counts), 50),]
```
...
start <- Sys.time()
de.lfcse <- lapply(models, function(x){TestModels(x, para = T)})  
...
names(de.lfcse)
GetSeqData <- function(de_nested_list, column){
  lapply(names(de_nested_list), function(x){
    lapply(de_nested_list[[x]], function(y){
      
    })
  })
}
de.lfcse[["unadjusted"]][[6]][["padj"]]
```
...
result <- map2_df(names(de.lfcse), de.lfcse, process_sub_list)

colnames(result) <- c("model_name", "n_de", "ave_se")
...
#write.csv(result, "jensen/results/benchmarking/deseq/08102023_1000g")
```


```{r get logfcSE, fig.width= 12, fig.height= 7}

se.res <- melt(result, id.vars = c("pct.change", "model_name", "ave_se", "n_de"))

se.res$NumberCovariates <- rep(c(0,1,2,3,1,2,3,1,2), each = length(se.res$model_name)/length(unique(se.res$model_name))) 

se.res$Model <- rep(as.character(models), each = length(se.res$model_name)/length(unique(se.res$model_name))) 
se.res$n_de[se.res$n_de == 0] <- 0.01


se.res <- se.res |> 
      mutate(ModelType = factor(case_when(
      str_detect(model_name, "unadjusted") ~ "No adjustment",
      str_detect(model_name, "raw") ~ "Raw proportions",
      str_detect(model_name, "clr") ~ "Centered log ratios",
      str_detect(model_name, "pc") ~ "PCA")))

pal <- c("#969696", "#fa9fb5", "#f768a1", "#c51b8a", "#a6bddb", "#74a9cf", "#2b8cbe", "#74c476", "#31a354")

se.res |>
subset(ModelType != "PCA") |>
ggplot(aes(x = pct.change, y = n_de + 0.1, color = Model)) +
