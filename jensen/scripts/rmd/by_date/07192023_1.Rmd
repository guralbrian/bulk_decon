...
```{r merge immune, echo=FALSE,warning=FALSE, include=TRUE, fig.height = 8, fig.width = 12}

cells.merge.immune <- levels(sn.clust)
names(cells.merge.immune) <- cells.merge.immune
cells.merge.immune[cells.merge.immune == "`B cells`" | cells.merge.immune == "granulocytes"] <- "immune"
sn.clust <- RenameIdents(sn.clust, cells.merge.immune)
...
wrap_plots(plots.new, 
           ncol = 3)
SaveH5Seurat(sn.clust, "jensen/data/processed/single_cell/merged_datasets/07192023_1")
```
## Bisque\
...
n_cells <- 10000
cells <- sample(colnames(sn.clust), n_cells, replace = FALSE)
sn.clust$ident <- Idents(sn.clust)
seurat <- sn.clust 
...
marker_genes <- left_join(ratios, fc, by = c("gene", "cellType.target")) |>
...
#write.csv(top_markers, "jensen/results/estimates/deconbuddies_marker_test_07102023_markers")

```

```{r immune + FB markers, fig.width = 40, fig.height= 30, echo=FALSE, message=FALSE, warning=FALSE}
...
markers <- read.csv("jensen/results/estimates/deconbuddies_marker_test_07102023_markers", row.names = 1)

immune_markers <- markers |>
                  group_by(cellType.target) |>
                  subset(cellType.target == "immune") |>
                  arrange(-logFC)|>
                  slice_head(n = 10)

genes <- immune_markers$gene
names(genes) <- c(immune_markers$anno_ratio)
...
genes.fib <- fib_markers$gene
names(genes.fib) <- c(fib_markers$anno_ratio)

genes <- c(genes, genes.fib)
...
props.music.markers <- EstimateCellTypeProportions(sn.clust, bulk.es.exp, for.aitchison = F, bulk.log2 = F,
                                                   sn.individuals = "orig.ident", marker = marker_genes, cells_exclude = NULL) #"endocardium") 
props.music.all <- EstimateCellTypeProportions(sn.clust, bulk.es.exp, for.aitchison = F, 
                                               sn.individuals = "orig.ident", bulk.log2 = F, marker = NULL, cells_exclude = NULL) #"endocardium") 
```
...
decon.melt <- rbind(props.music.markers, props.music.all) #|> rbind(props.bisque)
...
model.1 <- DirichReg(CellTypes ~ treatment * genotype, data = dir.mat)

model.2 <- DirichReg(CellTypes ~ treatment * genotype, data = dir.mat, model = "alternative", base = 1)

model.3 <- DirichReg(CellTypes ~ treatment * genotype, data = dir.mat, model = "alternative", base = 2)

model.4 <- DirichReg(CellTypes ~ treatment * genotype, data = dir.mat, model = "alternative", base = 3)

model.5 <- DirichReg(CellTypes ~ treatment * genotype, data = dir.mat, model = "alternative", base = 4)
...
# model.5 uses the endocardium as reference and is able to assign significance of the gene x treatment interaction to all of the other cell types
#write.csv(decon.melt, "jensen/results/estimates/deconbuddies_marker_test_07102023")
```