...
sn.small <- sn |>
  ...
  as.matrix() |>
  SampleUMI(5000, verbose = T, upsample = T)
...
```{r plot DESeq output, fig.width=10, fig.height= 6}
...
pal <- c("#fa9fb5", "#f768a1",  "#74c476", "#a6bddb", "#74a9cf", "#969696")
# Modify legend text 

models.legend <- list(
  unadjusted    = ~ "Group",
  raw_cm        = ~ "Group + CM proportion",
  raw_cm_fb     = ~ "Group + CM proportion + FB proportion",
  #raw_cm_fb_im  = ~ 0 + pct.change + cardiomyocytes + fibroblasts + immune,
...
ggplot(aes(x = sub_sublist_name, y = false_pos +0.01, color = sublist_name)) +
  ...
  geom_magnify(from = c(-0.07, 0.07,-3,15), to = c(-0.08,0.08, 150,300), 
                   shape = "rect", shadow = F)  +
  labs(y= "False Positives per 1000 genes", x = "Simulated fibroblast proportion difference") +
  ggtitle("Compositional covariates in DE analysis minimize false-positives")
