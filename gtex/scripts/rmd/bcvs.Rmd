...
sn.clust <- sn.all |>
    FilterByQuantile() |>
    ClusterSeurat(subset = F, res = 0.05, regress.by = c("Participant.ID"))
...
bulk.es <- exprs(bulk.es)
props.music.all <- EstimateCellTypeProportions(sn.labeled, bulk.es, for.aitchison = T, sn.individuals = "Participant.ID", marker = rownames(bulk.qc))$Est.prop.weighted |>
                  melt()
...
```{r plot gtex all, fig.width = 12, fig.height= 18, echo=FALSE, message=FALSE, warning=FALSE}
library(forcats)
